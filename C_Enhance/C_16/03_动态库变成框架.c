...
int myEncData(unsigned char *inData, int inDataLen, unsigned char *outData, int *outDataLen, void *refData, int refDataLen)
{
...
int myEncData2(unsigned char *inData, int inDataLen, unsigned char *outData, int *outDataLen, void *refData, int refDataLen)
{
	memcpy(outData, "111111111111111111111111111111111", 10);
	*outDataLen = 10;
	return 0;
...
// 动态库调试不能通过新建空项目调试，必须新建控制台空项目，原因未知
int main03()
{
	int				rv = 0;
	void			*handle = NULL;
